...
프로덕션에서도 관리자 로그인을 쓰려면 **Web 애플리케이션** OAuth 클라이언트가 따로 필요하다:

1. Google Cloud Console → Credentials → Create Credentials → OAuth client ID → **Web application**
2. **Authorized redirect URIs** 에 추가: `https://calendar.viram.dev/auth/callback`
3. 받은 JSON 으로 로그인용 자격증명을 교체/추가
   - 가장 간단: 이 Web 클라이언트로 로컬에서 캘린더 토큰까지 다시 발급(`login.py`)하면
     `credentials.json`/`token.json` 하나로 캘린더 읽기 + 관리자 로그인 모두 커버
   - (리프레시 토큰은 클라이언트마다 다르므로, 클라이언트를 바꾸면 `token.json` 재발급 필요)
4. 그 계정 이메일이 `.env` 의 `ADMIN_EMAILS` 에 있어야 관리자로 인정

> 당장은 관리자 큐레이션을 **로컬에서** 하고 `categories.json` 만 서버로 복사해도 된다.
